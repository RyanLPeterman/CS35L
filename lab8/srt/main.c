...
// There are global so that the threads can access it from their function
// number of threads passed in through args
int num_threads;
scene_t scene;
// the associated rgb of every pixel
float scaled_color[height][width][3];


// thread function for when we call pthread_create
void* multithread_ray_trace(void* thread_num_arg) {

    int* thread_num = (int*) thread_num_arg;

    // this heavy computation was cut from main
    Vec3 camera_pos;
    ...
    // Now instead of every pixel we pass this thread only
    // every num_threads pixel since the other threads will handle the others
    for( int px = *thread_num; px < width; px += num_threads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            // store the final calculated values into our global pixel color array
    ...
    // return since the result was already stored in our global pixel array
    return NULL;
}
...
{
    // changed to global for use in thread functions
    num_threads = argc == 2 ? atoi( argv[1] ) : 0;
    ...
    // allocate memory for all thread ids
    ...
    // allocate memory for pointers to thread function arg
    int* row_num = (int*) malloc(num_threads * sizeof(int));
    if(row_num == NULL) {
        fprintf(stderr, "Error: Could not allocate memory for thread args");
        exit(1);
    }
    ...
    for(i = 0; i < num_threads; i++) {
        if(pthread_create(thread_ids + i, 0, multithread_ray_trace, &row_num[i])) {
    ...
    // print the results out to disk, this depends on ordering therefore
    // we cannot run this until all the threads finish
